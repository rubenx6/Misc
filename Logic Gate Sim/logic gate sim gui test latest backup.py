...
winsurface = pg.display.get_surface()
...
def SaveCircuit(): #save circuit date to file
    ...
    if gqnt != 0:
        ...
        file.write("\n")
    for i in range(wqnt):
        file.write('{0},{1},{2},{3}\n{4}'.format(allwires[i].incon,allwires[i].outcon,allwires[i].ioStatus,allwires[i].fromsource,allwires[i].position))
    file.close()
...
while run:
        ...
        if event.type == pg.MOUSEBUTTONDOWN:
            if event.button == 3: #right-click
                if inready or outready:
                    for i in range(wqnt):
                        if not allwires[i].isSet: #cleanup wires with right-click               
                            wqnt -= 1
                            inready = False
                            outready = False
            ...
            if event.button == 1: #left-click
                if m.object[9].collidepoint(event.pos):
                     SaveCircuit()
                if m.object[10].collidepoint(event.pos):
                     x.clear()
                     allinputs.clear()
                     alloutputs.clear()
                     allwires.clear()
                     ...
                     try:
                         ...
                         allwires.clear()
                         iqnt,oqnt,gqnt,wqnt = 0,0,0,0
                if m.object[11].collidepoint(event.pos):
                     x.clear()
                     allinputs.clear()
                     alloutputs.clear()
                     allwires.clear()
                     iqnt,oqnt,gqnt,wqnt = 0,0,0,0
                for i in range(len(gatelist)):
                    if event.pos[0] > m.object[i+2].center[0] - 50 and event.pos[0] < m.object[i+2].center[0] + 50:
                        if event.pos[1] >= m.object[i+2].center[1] - 50 and event.pos[1] <= m.object[i+2].center[1]  + 50:
                            x.append(gate(gatelist[i],event.pos[0],event.pos[1]))
                            gqnt += 1
                            x[gqnt-1].ID = gqnt-1
                if event.pos[0] > m.object[0].center[0] - 35 and event.pos[0] < m.object[0].center[0] + 35:
                     if event.pos[1] > m.object[0].center[1] - 35 and event.pos[1] < m.object[0].center[1] + 35:
                         allinputs.append(source(event.pos[0],event.pos[1]))
                         iqnt += 1
                if event.pos[0] > m.object[1].center[0] - 35 and event.pos[0] < m.object[1].center[0] + 35:
                     if event.pos[1] > m.object[1].center[1] - 35 and event.pos[1] < m.object[1].center[1] + 35:
    ...
    if keys[pg.K_ESCAPE]: #cleanup wires with ESC       
        for i in range(wqnt):
